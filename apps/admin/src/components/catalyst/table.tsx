      ...
      <div className="flow-root">
        ...
        >
          <div className={clsx('inline-block min-w-full align-middle', !bleed && 'sm:px-(--gutter)')}>
            <table className="min-w-full text-start text-sm/6 text-zinc-950 dark:text-white">
              {children}
            </table>
          </div>
        </div>
...
export function TableRow({
  ...
  return (
    <TableRowContext.Provider
      value={{ href, target, title, onNavigate } as React.ContextType<typeof TableRowContext>}
    >
      <tr
        {...props}
        className={clsx(
...
export function TableCell({
  ...
  const { href, target, title, onNavigate } = useContext(TableRowContext)
  ...
  return (
    <td
      ...
      className={clsx(
        className,
        'relative px-4 first:ps-(--gutter,--spacing(2)) last:pe-(--gutter,--spacing(2))',
